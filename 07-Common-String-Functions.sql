...
SELECT LOWER(@string) AS LowerCaseNam

-- Extracting the first 10 characters of each name using SUBSTRING
SELECT SUBSTRING(@string, 1, 10) AS NameSubstring
...
SELECT LTRIM(@string) AS NameWithNoLeadisngSpaces
...
SELECT LTRIM(RTRIM(@string)) AS TrimmedName

-- Removing both leading and trailing spaces from the employee's name using TRIM
SELECT TRIM(@string) AS NameWithNoSpaces
